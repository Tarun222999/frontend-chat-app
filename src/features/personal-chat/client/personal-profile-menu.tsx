...
export function PersonalProfileMenu({
  session,
  compact = false,
}: {
  ...
  const triggerPaddingClass = compact ? "p-1" : "p-1.5"
  const avatarSizeClass = compact ? "h-8 w-8 text-sm" : "h-10 w-10"
  ...
  return (
    <details className="relative">
      <summary
        ...
        role="button"
        className={`list-none rounded-full border border-zinc-800 bg-zinc-950/80 text-white transition-colors hover:border-cyan-400 [&::-webkit-details-marker]:hidden ${triggerPaddingClass}`}
      >
        <span
          className={`flex items-center justify-center rounded-full bg-cyan-400 font-semibold text-slate-950 ${avatarSizeClass}`}
        >
